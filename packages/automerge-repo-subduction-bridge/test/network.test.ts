...
import { PeerId, Message } from "@automerge/automerge_subduction"
...
describe("NetworkAdapterConnection", () => {
    describe("basic connectivity", () => {
        ...
        it("can disconnect", async () => {
            ...
            await expect(
                connection.send(Message.blobsRequest([]))
    ...
    describe("recv() blocking behavior", () => {
        it("recv() blocks until a message arrives", async () => {
            ...
            const alicePeerId = new PeerId(
                crypto.getRandomValues(new Uint8Array(32))
            )
            const bobPeerId = new PeerId(
                crypto.getRandomValues(new Uint8Array(32))
            )
