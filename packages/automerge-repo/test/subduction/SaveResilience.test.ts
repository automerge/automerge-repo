...
describe("SubductionSource #save resilience", () => {
  ...
  it("saveInProgress is cleared even if subduction storage writes reject", async () => {
    ...
    // Matches the bridge's DEFAULT_PREFIX (storage.ts): the "sdn" namespace
    // marks the blob-inlining + compact-key storage format.
    const storage = new SelectivelyRejectingStorageAdapter("sdn")
    const repo = new Repo({ storage, network: [] })
  ...
  it("transient addBatch rejection does not lose commits — retry persists everything", async () => {
    ...
    const storage = new TransientlyRejectingStorageAdapter("sdn", 2)
    const repo = new Repo({ storage, network: [] })
    ...
    try {
      ...
      // stable.
      const commitCount = await storage.innerCount(["sdn", "c"])
