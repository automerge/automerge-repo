...
describe("Network subsystem", () => {
  const setup = ({ startReady = true } = {}) => {
    ...
    const peerMetadata: Promise<PeerMetadata> = Promise.resolve({ storageId: "no-such-id" as StorageId, isEphemeral: true })
  ...
  it("can be instantiated with a network adapter", async () => {
    ...
    assert(network instanceof NetworkSubsystem, "Network should be an instance of NetworkSubsystem")
  })
  ...
  it("should become ready with a ready adapter", async () => {
    ...
    assert.strictEqual(network.isReady(), true, "Network should be ready with a ready adapter")
  })

  it("allows adding a network adapter after creation", async () => {
    const { networkAdapter, peerId, peerMetadata } = setup({ startReady: false })
    const network = new NetworkSubsystem([], peerId, peerMetadata)
  ...
  it("does not become ready prematurely when adapter is not ready", async () => {
    ...
    const network = new NetworkSubsystem([], peerId, peerMetadata)
    network.addNetworkAdapter(networkAdapter)
    assert.strictEqual(network.isReady(), false, "Network should not be ready immediately when adapter is not ready")
  })
  ...
  it("allows removing a network adapter after creation", async () => {
    ...
    assert(network !== null)
    assert.strictEqual(network.isReady(), false, "Network should not be ready after removing the adapter")
  })
  ...
  it("handles ready behaviour for multiple network adapters correctly", async () => {
    ...
    await network.whenReady()
    assert.strictEqual(network.isReady(), true, "Network should be ready when all adapters are ready")

    network.removeNetworkAdapter(networkAdapter)
    assert.strictEqual(network.isReady(), true, "Network should remain ready when at least one adapter is still ready")
  })
})
