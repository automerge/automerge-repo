export default class Repo extends EventTarget {
    ...
    constructor(storage, network, url) {
        ...
        ;[syncState, msg] = Automerge.generateSyncMessage(this.docs[docId], Automerge.initSyncState())
        ...
        const onMessage = (peerId, docId, message) => {
            ...
            this.dispatchEvent(new CustomEvent('change', { detail: { docId, doc }, origin: 'remote' }))
        }
    ...
    change(docId, callback) {
        ...
        this.save(docId, doc)
        this.syncWithPeers(docId, doc)
        this.dispatchEvent(new CustomEvent('change', { detail: { docId, doc }, origin: 'local' }))
